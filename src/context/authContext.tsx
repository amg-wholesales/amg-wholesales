...
export function AuthProvider({ children }) {
  ...
  const logout = async () => {
    try {
      ...
      if (data.success) {
        ...
        if (currentUserType === 'ADMIN') {
            window.location.href = '/auth/admin/login';
        } else if (currentUserType === 'WHOLESALE_BUYER' || currentUserType === 'RETAIL_BUYER') {
            window.location.href = '/auth/user/login';
        }
      } else {
        console.error('Logout failed:', data.error);
  ...
  const value = {
    ...
    isAuthenticated: !!userId,
    isAdmin: userType === 'ADMIN',
    isWholesaleBuyer: userType === 'WHOLESALE_BUYER',
    isRetailBuyer: userType === 'RETAIL_BUYER',
    isBuyer: userType === 'WHOLESALE_BUYER' || userType === 'RETAIL_BUYER',
    isLoading,
    login,
    logout
  };
