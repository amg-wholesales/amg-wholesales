      ...
      <div className="flex-1 overflow-y-auto p-4">
        {cart.items.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-1 uppercase">Your cart is empty</h3>
            <p className="text-gray-500 mb-4">Add items to your cart to get started</p>
            <button
              onClick={onClose}
              className="bg-black text-white py-2 px-6 font-medium hover:bg-gray-800 flex items-center"
            >
              <Plus className="h-4 w-4 mr-2" /> Continue Shopping
            </button>
          </div>
        ) : (
          <ul className="divide-y divide-gray-200">
            {cart.items.map((item) => (
              <li key={item.id} className="py-6 flex">
                {/* Product Image */}
                <div className="h-24 w-24 flex-shrink-0 overflow-hidden rounded-none border border-gray-200 relative">
                  {item.product.image ? (
                    <img
                ...
                <div className="ml-4 flex flex-1 flex-col">
                  <div>
                    <div className="flex justify-between text-base font-bold text-gray-900">
                      <h3 className="uppercase">{item.product.name}</h3>
      ...
      <div className="border-t border-gray-200 p-4 space-y-4 bg-gray-50">
        ...
        <div className="mt-6 space-y-3">
          <button
            ...
            className={`w-full flex justify-center items-center py-3 px-4 rounded-none shadow-sm text-base font-medium text-white ${
          ...
          <button
            onClick={onClose}
            className="w-full flex justify-center items-center py-3 px-4 border border-gray-300 rounded-none shadow-sm text-base font-medium text-gray-700 bg-white hover:bg-gray-50"
          >
            Continue Shopping
          </button>
